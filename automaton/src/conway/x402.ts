...
// USDC contract addresses (primary)
...
// Explicit RPC URLs for chains whose default RPCs are unreliable
...
export async function getUsdcBalanceDetailed(
  ...
  try {
    const rpcUrl = RPC_OVERRIDES[network];
    const client = createPublicClient({
      chain,
      transport: http(rpcUrl),
    });

    // Read primary USDC contract
    const primaryBalance = await client.readContract({
      address: usdcAddress,
      abi: BALANCE_OF_ABI,
    ...
    // Read any extra USDC contracts (e.g. bridged USDC.e on Polygon)
    let extraBalance = 0n;
    const extras = USDC_EXTRA_ADDRESSES[network];
    if (extras) {
      ...
      for (const r of results) {
        if (r.status === "fulfilled") {
          extraBalance += r.value;
        }
      }
    }

    // USDC has 6 decimals
    const total = Number(primaryBalance + extraBalance) / 1_000_000;
    return {
      balance: total,
      network,
      ok: true,
